...
// trim from end
inline std::string RightTrim(std::string s, std::string trim = " \t\r\n") {
	std::string str(s);
...
inline std::string ExtractDirectory(const std::string path) {
	std::string filepath = path.substr(0, path.find_last_of(FOLDERSEPARATOR) + 1);
	return filepath;
}
inline std::string ExtractFilename(const std::string path) {
	std::string filename = path.substr(path.find_last_of(FOLDERSEPARATOR) + 1);
	return filename;
}
...
inline std::string ChangeExtension(const std::string& path,
...
inline std::vector<std::string> &Split(const std::string &s, char delim,
		std::vector<std::string> &elems, bool bKeepEmpty = false) {
...
inline std::string GetIniProfileString(std::string filename,
	...
	std::map<std::string, std::string> symbols = ParseIniSection(filename,
			section);
	return symbols[key];
}
template<typename T>
...
inline std::vector<std::string> Tokenizer(const std::string& str,
		const std::string& delimiters, bool bKeepToken = false) {
	std::vector<std::string> tokens;
	std::string::size_type delimPos = 0, tokenPos = 0, pos = 0;
...
inline std::string FindPattern(std::string & inStr, std::string oldBeginningStr,
		std::string oldEndingStr) {
	size_t i, j, k;
